...
vi.mock('@/utils/fetch-html', () => ({
  default: vi.fn((url: string) =>
    Promise.resolve({
      html: '<html><body><article>测试文章内容</article></body></html>',
      url,
    })
  ),
}))
...
describe('G: UserReadLaterController', { sequential: true }, () => {
  ...
  test('create unread item from url', async () => {
    const row = await UserReadLaterController.createFromUrl({ url: faker.internet.url() })
  ...
  test('different users can create same url', async () => {
    ...
    await expect(UserReadLaterController.createFromUrl({ url })).resolves.toMatchObject({ url })
  })
  ...
  test('create item with fallback content when fetching html fails', async () => {
    const url = faker.internet.url()
    vi.mocked(fetchHtml).mockRejectedValue(new Error('获取 HTML 失败'))

    const row = await UserReadLaterController.createFromUrl({ url })

    expect(row.status).toBe('unread')
    expect(row.url).toBe(url)
    expect(row.title).toBe(new URL(url).hostname)
    expect(row.summary).toContain('暂时无法获取页面 HTML')
    expect(row.estimatedReadingMinutes).toBe(1)
  })
  ...
  test('read and unread items can be deleted', async () => {
    ...
    const read = await UserReadLaterController.markRead({ id: unread.id })
